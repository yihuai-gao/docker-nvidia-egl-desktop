...
RUN apt-get clean && apt-get update && apt-get dist-upgrade -y && apt-get install --no-install-recommends -y \
    ...
    groupadd -g ${USER_GID} ${USERNAME} || echo 'Failed to add ${USERNAME} group' && \
    useradd -ms /bin/bash ${USERNAME} -u ${USER_UID} -g ${USER_GID} || echo 'Failed to add ${USERNAME} user' && \
    usermod -a -G adm,audio,cdrom,dialout,dip,fax,floppy,games,input,lp,plugdev,render,ssl-cert,sudo,tape,tty,video,voice ${USERNAME} && \
    echo "${USERNAME} ALL=(ALL:ALL) NOPASSWD: ALL" >> /etc/sudoers && \
    echo "${USERNAME}:${PASSWD}" | chpasswd && \
    chown -R -f -h --no-preserve-root ${USERNAME}:${USERNAME} / || echo 'Failed to set filesystem ownership in some paths to ${USERNAME} user' && \
...
# # Copy scripts and configurations used to start the container with `--chown=${USER_UID}:${USER_GID}`
...
# COPY --chown=${USER_UID}:${USER_GID} kasmvnc-entrypoint.sh /etc/kasmvnc-entrypoint.sh
...
RUN /home/${USERNAME}/miniforge3/bin/mamba create -n "mujoco-env" python=3.10.15 mujoco=3.2.5 && \
